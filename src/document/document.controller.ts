...
export class DocumentController {
  ...
  async upload(@UploadedFile() file: Express.Multer.File, @Req() req) {
    this.logger.log(
      `[Upload] file  : ${file.originalname}  fileSize : ${file.size}  type of : ${file.mimetype}  `,
    );
    const ip = req.ip;
  ...
  async convert(@Body() convertDTO: ConvertDTO) {
    this.logger.log(
      `[Convert] URL : ${convertDTO.url} from [${convertDTO.from}-${convertDTO.fromType}] to [${convertDTO.to}-${convertDTO.toType}]`,
    );
